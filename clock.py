...
class Clock():
    ...
    def update_clock(self):
        ...
        # Draw clock face
        self.canvas.create_oval(2, 2, WIDTH, HEIGHT, outline="black", width=2)
        ...
        for i in range(12):
            ...
            if i == 0:
                self.canvas.create_text(x, y-10, text=str(i+12), font=("Helvetica", 12))
            else:
        ...
        self.canvas.create_line(WIDTH/2, HEIGHT/2, hour_x, hour_y, fill="black", width=6)
        ...
        self.canvas.create_line(WIDTH/2, HEIGHT/2, minute_x, minute_y, fill="black", width=4)
    
        # Draw second hand
        second_angle = second * math.pi/30 - math.pi/2
        second_x = WIDTH/2 + 0.6 * WIDTH/2 * math.cos(second_angle)
        second_y = HEIGHT/2 + 0.6 * WIDTH/2 * math.sin(second_angle)
        self.canvas.create_line(WIDTH/2, HEIGHT/2, second_x, second_y, fill="red", width=2)
